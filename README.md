...
- New accounts get an optional, resumable welcome flow after choosing a name: connect with their inviter, add a picture, choose colours and topics, set their update and notification pace, and optionally enable notifications on this device. Continue saves each step’s choices, even if nothing changed. The final Make yourself at home button completes the flow. Progress is saved in account metadata; notification choices use the existing preferences, and colours remain browser-local. Existing profiles are not enrolled automatically. All choices remain available in **Your preferences**. Afterwards, they are guided to **The parlor**. Save the generated recovery code in a password manager and choose whether this is a trusted personal device. There is no separate messaging password to create. Then select a friend and send a note.
...
- Test on another device by signing in and entering the saved recovery code. Existing accounts can still use their original messaging passphrase.
