...
test('Parlor sorts kin, highlights inner circle, searches without losing focus and selects while locked', async()=>{
 ...
 const tables={hearth_kin_groups:[{id:'family',name:'Family'}],hearth_kin_group_members:[],hearth_profiles:people,hearth_connections:[{requester:me,recipient:zoe,accepted:true},{requester:amy,recipient:me,accepted:true}],hearth_circle:[{owner:me,member:amy}],hearth_preferences:{topics:[],update_mode:'manual'}};
 let savedPlan;
 const client={auth:{onAuthStateChange(){},async getSession(){return{data:{session:{user:{id:me}}}}}},async rpc(action,args){if(action==='hearth_save_event')savedPlan=args;return{data:action==='hearth_sent_introductions'?[{id:'sent',a:zoe,b:amy,a_name:'Zoe',b_name:'amy',created_at:'2026-09-17T10:00:00Z'}]:action==='hearth_event_attendees'?[]:null}},from(table){let single=false;const q={select(){return q},eq(){return q},neq(){return q},order(){return q},gte(){return q},limit(){return q},range(){return q},in(){return q},maybeSingle(){single=true;return q},then(resolve){return Promise.resolve({data:table==='hearth_profiles'&&single?people[0]:tables[table]??(single?null:[])}).then(resolve)}};return q}};
